        ...
        while (true){
            ...
            switch (eingabe){
                case "1":
                    ...
                    /*
                    1. Eine Eingabeaufforderung der Nummer in der Liste was gelöscht werden soll.
                    2. Abfrage ob es den Index der Nummer überhaupt gibt.
                    3. Noch eine Abfrage wenn es den Index gibt ob wir den Eintrag wirklich löschen wollen.
                    4. Eintrag aus der Liste löschen.
                    */
                case "2":
                    System.out.print("Geben Sie bitte die Nummer vom Auftrag, den Sie löschen wollen, ein: ");
                    int userNummer = scanner.nextInt()-1;
                    scanner.nextLine();
                    if (userNummer > 0 && userNummer < elemente.size()){
                        System.out.print("Wenn Sie sicher sind, dass Sie diese Nummer löschen wollen, geben bitte 'j': ");
                        String antwort = scanner.nextLine();
                        if (antwort.equalsIgnoreCase("j")){
                            toDoListe.entferneElement(userNummer);
                        }
                        else {
                        ...
                        System.out.println("Diese Nummer existiert nicht!");
                    }
                    break;
                    case "3":
                        System.out.print("Möchten Sie alle löschen? Wenn ja - schreiben j");
                        String bestaetigungAlles = scanner.nextLine();
                        if (bestaetigungAlles.equalsIgnoreCase("j")){
                            toDoListe.entferneAlleElmente();
                        }
                        break;
                    case "4":
                            List<Integer> indizes = new ArrayList<>();
                            System.out.println("Geben Sie Nummern der Einträge ein die Sie löschen wollen getrennt durch Komas: (z.B 1,2,3)");
                            String [] eingabeIndizes = scanner.nextLine().split(",");
                            for (int i = 0; i < eingabeIndizes.length; i++){
                                try {
                                    int index = Integer.parseInt(eingabeIndizes[i]);
                                    if (index >= 0 && index < elemente.size()){
                                        indizes.add(index);
                                    }
                                    else System.out.println("Index "+ (index+1)+ " ist ungültig!");
                                } catch (NumberFormatException e){
                                    System.out.println("Ungültige Eingabe");
                                }
                            }
                            if (!indizes.isEmpty()){
                                System.out.println("Möchten Sie die ausgewählten Einträge wirklich löschen? ja/nein");
                                String bestaetigung = scanner.nextLine();
                                if (bestaetigung.equalsIgnoreCase("ja")){
                                    toDoListe.entferneMehrereElemente(indizes);
                                }
                            }
                            break;
                    case "5": // Eintrag ändern
                        System.out.print("Geben Sie die Nummer des Eintrags ein, den Sie ändern möchten: ");
                        int indexAendern = scanner.nextInt()-1;
                        scanner.nextLine();
                        if(indexAendern >= 0 && indexAendern < elemente.size()){
                            System.out.print("Geben Sie bitte die neue Aufgabe ein: ");
                            String neueAufgabeAendern = scanner.nextLine();
                            toDoListe.aendereElement(indexAendern, neueAufgabeAendern);
                        }
                        break;
                    case "6":
                        toDoListe.speichere();
                        System.out.println("Das Programm wird beendet, einen schönen Tag noch.");
                        scanner.close();
                        return;

                default:
                            System.out.println("Ungültige Eingabe, versuchen nochmal");
